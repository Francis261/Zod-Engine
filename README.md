...
3. Builds a planning prompt from node tags + project structure + summaries + change logs + tool rules (without sending full codebase)
4. Runs iterative `/api/ai-call` steps and parses tool tags until done/max steps
5. Applies file operations and regenerates/persists project structure after each step
6. Updates selected nodes (`brain_write`, `brain_history`)
7. Returns AI response + selected nodes + task trace + operations
...
- Send button
- Displays project snapshot (structure + file list), AI response, execution summary, task trace, context manifest, applied operations, history summaries, change logs, and selected nodes
